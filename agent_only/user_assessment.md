...
- **Self-catching bugs before reporting done**: Sessions 1, 4, 5, 6, 7, and 8 all had bugs not self-caught. 6 of 11 sessions. Session 8: typo and wrong boundary direction — neither caught before asking for feedback. Session 11 (LC 160) is the first counter-example: no logical bugs at all, and the one stylistic gap (`!=` vs `is not`) was self-identified during blog writing, unprompted. Worth watching whether this generalizes or was specific to an easier problem (LC 160 is rated Easy, unlike most recent sessions).
...
- Self-catching bugs — 6 of 11 sessions with uncaught bugs. Sessions 2, 9, 10 (final attempt), and 11 were bug-free. Session 11 additionally showed self-catching of a stylistic gap during blog writing, unprompted — first such instance. Next session with a real logical bug: still hold back all feedback until the user has traced a concrete example themselves. Do not skip.
- Correctness argument in blog — blank on initial submission in most sessions. Session 10 required 3 saves and explicit prompting. Session 11 broke this pattern: correctness argument was accurate and complete on first submission. Watch whether this holds on the next Medium/Hard problem, or was easier because LC 160 is rated Easy.
- Invariant articulation — LC 48 and LC 240 blogs both passed without explicitly naming the invariant as a formal statement. LC 160's correctness argument stated the m+n-c equalization precisely, which functions as an implicit invariant statement. Still worth testing for an explicit "invariant:" style statement on a future problem.
...
- Linked-list / two-pointer transfer — LC 160 was the first linked-list problem observed. Next step: a problem requiring two-pointer techniques on a single list (e.g., cycle detection LC 141/142, or middle-of-list) to see whether the pointer-synchronization instinct transfers to a different list-topology problem.
