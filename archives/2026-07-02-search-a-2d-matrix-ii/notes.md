...
- Initial attempt: row-by-row pruning loop (brute force with early row termination using row truncation). Incomplete — no equality check, no return value, would cause IndexError on truncated rows.
- Pivoted to staircase search before first complete submission. Zero bugs in final implementation.
- Blog: 3 revision cycles — initially had 4 blank sections, then English-only violation (Chinese text added and fixed), then correctness argument added on final prompt.
- Correctness argument was accurate once written.

---

## Coaching Targets (pre-session)
