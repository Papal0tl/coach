...
- Correctly answered (unprompted reasoning requested, not yet given) that divide-and-conquer merge order is O(N log k) vs. the sequential fold's O(Nk); asked to explain *why* (what the log k counts, why sequential fold doesn't get it) — answer still pending as of the next code edit.
